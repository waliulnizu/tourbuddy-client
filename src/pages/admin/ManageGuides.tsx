...
export default function ManageGuides() {
  ...
  const [form, setForm] = useState<GuideForm>({ name: '', designation: '', email: '', phone: '', status: 'active' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false); setEditing(null); setEditingGuide(null);
      setForm({ name: '', designation: '', email: '', phone: '', status: 'active' }); setFile(null); setPreview(null);
      fetchData();
    } catch (err: any) {
      console.error(err);
  ...
  const handleEdit = (g: Guide) => {
    setForm({ name: g.name, designation: g.designation || '', email: g.email || '', phone: g.phone, status: g.status || 'active' });
    setEditing(g._id);
    setEditingGuide(g);
    setFile(null);
    if (g.guide_image) setPreview(`${import.meta.env.VITE_API_URL}/${g.guide_image}`);
  ...
  const openAddForm = () => {
    ...
    setForm({ name: '', designation: '', email: '', phone: '', status: 'active' });
    setFile(null);
    setPreview(null);
  };
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 animate-fade-in-up">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl">
            ...
            <div>
              ...
              <label className="flex flex-col items-center justify-center gap-2 px-4 py-5 border-2 border-dashed border-gray-200 rounded-xl hover:border-blue-400 hover:bg-blue-50/50 transition-all cursor-pointer">
                <input type="file" accept="image/*" className="hidden" onChange={handleFileSelect} />
                {preview ? (
                  <img src={preview} alt="" className="w-16 h-16 object-cover rounded-full border-2 border-gray-100" />
                ) : (
                  <svg className="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                )}
                <span className="text-sm text-gray-500">{file ? file.name : editing ? 'Change image' : 'Click to upload'}</span>
              </label>
            </div>
            <div className="md:col-span-2">
                ...
                {guides.length > 0 ? guides.map((g) => (
                  <tr key={g._id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <img src={g.guide_image ? `${import.meta.env.VITE_API_URL}/${g.guide_image}` : 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=80&q=80'} alt="" className="w-10 h-10 object-cover rounded-full border border-gray-100" />
                    </td>
